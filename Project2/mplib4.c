...
=========================================================================
|  VERSAO 4 REALIZADA POR:
|  GRUPO 47:
|  - Joao Freitas, 87671
|  - Pedro Soares, 87693
|  LEIC-T, Exercicio 2 do Projeto, Turno 3ªfeira, 14:30-16:00 (SALA 0-14)
=========================================================================
*/
...
typedef struct channel_t {
  ...
  pthread_cond_t     my_wait_for_free_space;
  pthread_cond_t     my_wait_for_messages;
} Channel_t;
...
int inicializarMPlib(int capacidade_de_cada_canal, int ntasks) {
  ...
  for (i=0; i<ntasks; i++) {
    for (j=0; j<ntasks; j++) {
      ...
      if(pthread_mutex_init(&channel->my_mutex, NULL) != 0) {
        fprintf(stderr, "\nErro ao inicializar mutex\n");
        return -1;
      }

      if(pthread_cond_init(&channel->my_wait_for_free_space, NULL) != 0) {
        fprintf(stderr, "\nErro ao inicializar variável de condição\n");
        return -1;
      }

      if(pthread_cond_init(&channel->my_wait_for_messages, NULL) != 0){
        fprintf(stderr, "\nErro ao inicializar variável de condição\n");
        return -1;
      }
...
void libertarMPlib() {
  ...
  for (i=0; i<number_of_tasks; i++) {
    for (j=0; j<number_of_tasks; j++) {
      ...
      if(pthread_mutex_destroy(&channel->my_mutex) != 0) {
        fprintf(stderr, "\nErro ao destruir mutex\n");
        exit(EXIT_FAILURE);
      }

      if(pthread_cond_destroy(&channel->my_wait_for_free_space) != 0) {
        fprintf(stderr, "\nErro ao destruir variável de condição\n");
        exit(EXIT_FAILURE);
      }

      if(pthread_cond_destroy(&channel->my_wait_for_messages) != 0) {
        fprintf(stderr, "\nErro ao destruir variável de condição\n");
        exit(EXIT_FAILURE);
      }
...
int receberMensagem(int tarefaOrig, int tarefaDest, void *buffer, int tamanho) {
  ...
  if(pthread_mutex_lock(&channel->my_mutex) != 0) {
  ...
  while (!mess) {
    if(pthread_cond_wait(&channel->my_wait_for_messages, &channel->my_mutex) != 0) {
  ...
  if(pthread_cond_signal(&channel->my_wait_for_free_space) != 0) {
    fprintf(stderr, "\nErro ao desbloquear variável de condição\n");
    return -1;
  }

  // para bloquear uma thread num periodo de tempo maior e comparar com mplib3:
  // sleep(1);

  if(pthread_mutex_unlock(&channel->my_mutex) != 0) {
...
int enviarMensagem(int tarefaOrig, int tarefaDest, void *msg, int tamanho) {
  ...
  if (channel_capacity >0) {
    while (leQueSize(channel->message_list) >= channel_capacity) {
      if(pthread_cond_wait(&channel->my_wait_for_free_space, &channel->my_mutex) != 0) {
        fprintf(stderr, "\nErro ao esperar pela variável de condição\n");
        return -1;
      }
    }
  ...
  pthread_cond_signal(&channel->my_wait_for_messages);
